...
void Life::next(bool nextGen, bool drawNewCellsInGrid) {
        ...
        for (unsigned int i = 0; i < limit; i++) {
            unsigned int iM1 = (i == 0 ? limit - 1 : i - 1);
            unsigned int iP1 = (i == limit - 1 ? 0 : i + 1);

            for (unsigned int j = 0; j < limit; j++) {
                unsigned int jM1 = (j == 0 ? limit - 1 : j - 1);
                unsigned int jP1 = (j == limit - 1 ? 0 : j + 1);
                neighbors =
                        prevCells[ iM1 ][ j] +
                        prevCells[ iP1 ][ j] +
                        prevCells[ iM1 ][ jP1] +
                ...
                if ((neighbors == 3) && !prevCells[ i][ j]) { // any cell with exactly 3 neigbors comes to lives
                    nextCells[i][j] = true;
                    if (drawNewCellsInGrid) drawCell(i, j);
                    numLiveCells++;
                } else if ((neighbors == 2 || neighbors == 3) && prevCells[ i][ j]) { // any cell with exactly 2 or 3 stays alive
                    nextCells[i][j] = true;
                    if (drawNewCellsInGrid) drawCell(i, j);
                    numLiveCells++;
                }
