...
public class EvalNode {
	...
	public Product eval() throws EvaluationError {
		//
		Environment evalEnv = new Environment();
		evalEnv.addPropertyCollection(_sym.getEvaluablePropertySet());
		//
		if (_inputNodes != null) {
			for (int i = 0; i < _inputNodes.size(); i++) {
				evalEnv.addPropertyCollection(_inputNodes.get(i).eval().getEvaluablePropertySet("$" + (i + 1)));
			}
		}

		//+ checking validity before executing
		if (evalEnv.inspect() == Environment.CIRCULAR_DEPENDENCY) {
			throw new EvaluationError("CircularDependancy: The problem is most likely in the Lab");
		}
		if (evalEnv.inspect() == Environment.MISSING_DEPENDENCY) {
